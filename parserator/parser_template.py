...
try:
    ...
    warnings.warn('You must train the model (parserator train [traindata] '
                  '[modulename]) to create the %s file before you can use '
                  'the parse and tag methods' % MODEL_FILE)
...
def tokenize(raw_string):
    """ This determines how any given string is split into its tokens handle
        any punctuation you want to split on, as well as any punctuation
        to capture.
    """
...
def casing(token):
    """ Define any other methods for features. This is an example to get the
        casing of a token.
    """
    if token.isupper():
        return 'upper'
